    ...
    if (conversationId) {
      ...
      conversation = await db.query.conversations.findFirst({
        where: (conversations, { eq, and }) =>
          and(eq(conversations.id, conversationId), eq(conversations.userId, payload.userId)),
        with: {
          messages: {
...
app.delete('/api/protected/conversations', async c => {
  try {
    ...
    await db.delete(conversations).where(eq(conversations.userId, payload.userId));
  ...
  try {
    ...
    if (platformId && projectId) {
      ...
      const platform = await db.query.platforms.findFirst({
        where: (platforms, { eq, and }) =>
          and(eq(platforms.id, platformId), eq(platforms.userId, payload.userId)),
      });
...
app.post('/api/protected/platforms/:id/test', async c => {
  try {
    ...
    await db.update(platforms).set({ isActive: isConnected }).where(eq(platforms.id, platformId));
...
app.delete('/api/protected/platforms/:id', async c => {
  try {
    ...
    const result = await db
      .delete(platforms)
      .where(and(eq(platforms.id, platformId), eq(platforms.userId, payload.userId)));
...
app.delete('/api/protected/conversations/:id', async c => {
  try {
    ...
    const result = await db
      ...
      .where(and(eq(conversations.id, conversationId), eq(conversations.userId, payload.userId)));
