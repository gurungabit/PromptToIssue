...
export default function Chat() {
  ...
  const [projectsCache, setProjectsCache] = useState<{
    [platformId: string]: { data: Project[]; timestamp: number };
  }>({});
  const [milestonesCache, setMilestonesCache] = useState<{
    [key: string]: { data: Milestone[]; timestamp: number; version: number };
  }>({});
    ...
    try {
      ...
      if (conversationData.messages) {
        ...
        if (lastAiMessage?.metadata) {
          const metadata =
            typeof lastAiMessage.metadata === 'string'
  ...
  const openProjectSelector = async () => {
    try {
      ...
      if (platforms.length === 0) {
        addToast(
          'No platforms configured. Go to Settings to add GitLab or GitHub integration.',
          'warning'
        );
        return;
      }
  ...
  const loadMilestones = async (
    ...
    if (
      ...
      cached.version === MILESTONES_CACHE_VERSION
    ) {
    ...
    try {
      const response = await axios.get(
        `/api/protected/platforms/${platformId}/projects/${projectId}/milestones`
      );
...
${platformTickets
  .map(
    (ticket: PlatformTicket, index: number) =>
                        ...
                        <div className='leading-relaxed'>
                          ...
                          aiResponse?.tickets &&
                            ...
                            (message.content === aiResponse.response ||
                              (message.metadata &&
                                JSON.parse(message.metadata as string)?.tickets)) ? (
                            <AIResponseMessage
                            ...
                            <ReactMarkdown
                              ...
                              components={{
                                ...
                                p: ({ children }) => (
                                  <p className='text-gray-900 dark:text-white mb-2'>{children}</p>
                                ),
                                ul: ({ children }) => (
                                  <ul className='list-disc list-inside text-gray-900 dark:text-white mb-2 space-y-1'>
                                    {children}
                                  </ul>
                                ),
                                ol: ({ children }) => (
                                  <ol className='list-decimal list-inside text-gray-900 dark:text-white mb-2 space-y-1'>
                                    {children}
                                  </ol>
                                ),
                                li: ({ children }) => (
                    ...
                    <div className='w-8 h-8 bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg flex items-center justify-center'>
                      ...
                      >
                        <path
                          ...
                          strokeLinejoin='round'
                          strokeWidth={2}
                          d='M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h6a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h6a2 2 0 002-2v-4a2 2 0 00-2-2m8-8a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6z'
                        />
                      </svg>
                    </div>
                  ...
                  <div className='space-y-3 animate-in slide-in-from-bottom-2 duration-300'>
                    <div className='flex items-center justify-between'>
                      <div className='flex items-center space-x-2'>
                        ...
                        {selectedPlatform &&
                          selectedProject &&
                          milestonesCache[`${selectedPlatform}_${selectedProject}`] && (
                            <span className='text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-full'>
                              Cached (Open Only)
                            </span>
                          )}
                      ...
                      <button
                        onClick={() =>
                          selectedPlatform &&
                          selectedProject &&
                          loadMilestones(selectedPlatform, selectedProject, true)
                        }
                      ...
                      >
                        <svg
                          className={`w-4 h-4 ${loadingMilestones ? 'animate-spin' : ''}`}
                          fill='none'
                          stroke='currentColor'
                          viewBox='0 0 24 24'
                        >
