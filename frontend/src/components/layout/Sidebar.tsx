...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const { conversations, conversationsLoading, deleteConversation, deleteAllConversations } =
    useChat();
  ...
  const groupedConversations = processedConversations.reduce(
    (acc, conv) => {
      if (!acc[conv.date]) {
        acc[conv.date] = [];
      }
        ...
        <div className='flex flex-col h-full'>
          ...
          <div className='p-4 border-b border-gray-200 dark:border-gray-700'>
            <div className='flex items-center justify-between mb-4'>
              <h1 className='text-sm font-medium text-gray-900 dark:text-gray-100'>Navigation</h1>
              <button
                onClick={onClose}
