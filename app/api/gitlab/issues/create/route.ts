...
export async function POST(request: Request) {
  try {
    ...
    if (!ticket.projectId) {
      return new Response(
        JSON.stringify({ error: "Project ID is required to create an issue" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        },
    ...
    if (ticket.acceptanceCriteria && ticket.acceptanceCriteria.length > 0) {
      ...
      ticket.acceptanceCriteria.forEach(
        (ac: { completed: boolean; description: string }) => {
          descriptionParts.push(
    ...
    if (ticket.tasks && ticket.tasks.length > 0) {
      ...
      ticket.tasks.forEach(
        (task: {
          ...
          descriptionParts.push(
            `- [${task.completed ? "x" : " "}] ${task.description}${time}`,
          );
        },
      );
    }
    ...
    // 3. Map Assignees
    const assigneeIds =
      (ticket.assignees?.map((a) => a.id).filter(Boolean) as number[]) ?? [];
