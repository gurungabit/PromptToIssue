...
export function ArtifactRenderer({ content, chatId, messageId, onTicketCreated, onBulkTicketsCreated }: ArtifactRendererProps) {
  ...
  const handleBulkCreate = useCallback(async (assignments: TicketAssignment[]) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (onBulkTicketsCreated && data.results) {
          ...
          console.log('[ArtifactRenderer] Calling onBulkTicketsCreated with', successResults.length, 'results');
        ...
        if (part.type === 'ticket-response') {
          ...
          return (
            <div key={index} className="space-y-4 my-2">
              ...
              {part.data.needsClarification && part.data.clarificationQuestions && part.data.clarificationQuestions.length > 0 && (
                <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3 text-sm">
                  <p className="font-medium text-amber-800 dark:text-amber-200 mb-2">Clarification Needed:</p>
    ...
    <ReactMarkdown
      ...
      components={{
        code({ className, children, ...props }) {
          ...
          if (!match) {
            return (
              <code className="px-1.5 py-0.5 bg-zinc-200 dark:bg-zinc-800 rounded text-zinc-700 dark:text-zinc-300 text-[13px] font-mono" {...props}>
                {children}
              </code>
            );
          }
        ...
        h1({ children }) {
          return <h1 className="text-xl font-semibold mt-4 mb-2 text-zinc-900 dark:text-white">{children}</h1>;
        },
        h2({ children }) {
          return <h2 className="text-lg font-semibold mt-4 mb-2 text-zinc-900 dark:text-white">{children}</h2>;
        },
        h3({ children }) {
          return <h3 className="text-base font-semibold mt-3 mb-1.5 text-zinc-900 dark:text-white">{children}</h3>;
        },
        blockquote({ children }) {
          return (
