...
export function ModelSelector({
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {open && (
        <div
          className={`absolute left-0 w-72 py-1 border rounded-lg shadow-xl z-50 max-h-64 overflow-y-auto ${
          ...
          {models.map((model) => (
            ...
            >
              <div className="flex-1 min-w-0">
                <div
                  className={`text-sm font-medium whitespace-normal ${isDark ? 'text-white' : 'text-zinc-900'}`}
                >
                  {model.displayName}
                </div>
                {model.description && (
                  <div className="text-xs text-zinc-500 whitespace-normal line-clamp-2">
                    {model.description}
                  </div>
                )}
              </div>
