...
import { CheckCircle2, AlertCircle, Clock, ExternalLink, Plus, X, Trash2 } from 'lucide-react';
...
export function IssueWidget({
  ...
  async function createTicketWithProject(projectPath: string, milestoneId?: number) {
    ...
    try {
      ...
      console.log('[IssueWidget] Ticket created:', {
        webUrl: data.webUrl,
        title: ticketData.title,
        hasCallback: !!onTicketCreated,
      });
  ...
  function addTask() {
    setEditedIssue({
      ...editedIssue,
      tasks: [...editedIssue.tasks, { id: crypto.randomUUID(), description: '', completed: false }],
    });
  }
  ...
  function updateTask<K extends keyof Task>(id: string, field: K, value: Task[K]) {
    setEditedIssue({
      ...editedIssue,
      tasks: editedIssue.tasks.map((t) => (t.id === id ? { ...t, [field]: value } : t)),
    });
  }
  ...
  function addLabel(label: string) {
    console.log(
      '[IssueWidget] addLabel called with:',
      label,
      'current labels:',
      editedIssue.labels,
    );
    ...
    `}
      >
              ...
              {isEditing ? (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-3">
                    <select
                      ...
                      onChange={(e) =>
                        setEditedIssue({ ...editedIssue, type: e.target.value as Ticket['type'] })
                      }
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 text-xs text-zinc-500 mt-1.5">
                    <span className="font-mono bg-zinc-100 dark:bg-zinc-700/50 px-1.5 py-0.5 rounded">
                      #{editedIssue.iid || editedIssue.id.slice(0, 8)}
                    </span>
                    <span
                    ...
                    <span className="capitalize bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400 px-1.5 py-0.5 rounded-full">
                      {editedIssue.type}
                    </span>
                    ...
                    {editedIssue.projectId && (
                      <span className="text-zinc-400">
                        ...
                        <span className="text-blue-500 font-medium">
                          {selectedProjectName || editedIssue.projectId}
                        </span>
                      </span>
                    )}
                  </div>
        ...
        <div className="px-5 py-4 space-y-6">
          ...
          {/* Acceptance Criteria */}
          {(isEditing ||
            (editedIssue.acceptanceCriteria && editedIssue.acceptanceCriteria.length > 0)) && (
            <div className="space-y-3">
                ...
                {editedIssue.acceptanceCriteria.map((ac) => (
                  <div key={ac.id} className="flex items-start gap-2 group">
                    ...
                    {isEditing ? (
                      ...
                      <span
                        className={`text-sm ${ac.completed ? 'text-zinc-400 line-through' : 'text-zinc-700 dark:text-zinc-300'}`}
                      >
                        {ac.description}
                      </span>
                    )}
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                {editedIssue.tasks.map((task) => (
                  <div
                    key={task.id}
                    className="flex items-start gap-2 bg-zinc-50 dark:bg-zinc-800/50 p-3 rounded-lg border border-zinc-100 dark:border-zinc-700/50"
                  >
                    {isEditing ? (
