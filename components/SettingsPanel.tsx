...
export function SettingsPanel({ open, onClose }: SettingsPanelProps) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className={`fixed right-0 top-0 bottom-0 z-50 w-full max-w-md border-l shadow-2xl ${
        ...
        <div className={`h-14 flex items-center justify-between px-6 border-b ${
          ...
          <h2 className={`text-lg font-medium ${isDark ? 'text-white' : 'text-zinc-900'}`}>Settings</h2>
          <button
            onClick={onClose}
        ...
        {/* Content */}
        <div className={`p-6 space-y-8 overflow-y-auto h-[calc(100vh-56px)] ${
          ...
          {loading ? (
            ...
            <>
              {/* Theme Section */}
              <div>
                <h3 className={`text-sm font-medium mb-3 ${isDark ? 'text-white' : 'text-zinc-900'}`}>Theme</h3>
                {mounted && (
              ...
              {/* GitLab Integration */}
              <div>
                <h3 className={`text-sm font-medium mb-3 flex items-center gap-2 ${isDark ? 'text-white' : 'text-zinc-900'}`}>
                ...
                <p className="mt-2 text-xs text-zinc-500">
                  Connect GitLab to create issues, search projects, and manage your work directly from chat.
                </p>

                {/* MCP Toggle */}
                <div className={`mt-4 flex items-center justify-between py-3 px-4 rounded-lg border ${
                  isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'
                }`}>
                  <div>
                    <span className={`text-sm ${isDark ? 'text-white' : 'text-zinc-900'}`}>Enable MCP Tools</span>
              ...
              {/* Default Model */}
              <div>
                <h3 className={`text-sm font-medium mb-3 ${isDark ? 'text-white' : 'text-zinc-900'}`}>Default Model</h3>
                <select
              ...
              <div className={`pt-8 border-t ${isDark ? 'border-zinc-800' : 'border-zinc-200'}`}>
                <h3 className={`text-sm font-medium mb-2 ${isDark ? 'text-white' : 'text-zinc-900'}`}>About</h3>
                <p className="text-sm text-zinc-500">
                  Prompt2Issue - AI-powered chat with GitLab integration
                </p>
                <p className={`text-xs mt-1 ${isDark ? 'text-zinc-600' : 'text-zinc-400'}`}>Version 1.0.0</p>
              </div>
            </>
          )}
        </div>
